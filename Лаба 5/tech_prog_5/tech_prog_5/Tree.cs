...
namespace tech_prog_5
{
    public class Tree //Описание класса "Дерево"
    {
        ...
        {
            s = "Обычный вывод (левая ветв, правая ветьвь)"; // обнуляем строку
    ...
    private void OutputTree(TreeNode root, int spaces, ref string s)
    {
        if (root != null)
        {
            for (int i = 0; i < root.Count; i++)
            {
                ...
                s += " ";               
                if (Form1.chars_is_symb == true) s += $"(дл: {root.znacn.Length})";
        ...
        private void OutputTreeLayer(TreeNode root, int spaces, ref string s)
        {
            if (root != null)
            {
                for (int i = 0; i < root.Count; i++)
                {
                    ...
                    s += " ";
                    if (Form1.chars_is_symb == true) s += $"({root.znacn.Length})";
                    s += root.znacn.ToString();
                    s += Environment.NewLine;
                }
                OutputTree(root.Left, spaces + 1, ref s);
        ...
        ///пока что сделать сортировку просто по длине строк +
        ///есил строка имеет одинаковое количество символов, то разбить ее по символам и сложить численные значения символов
        ///Принципы упорядовочиния ветвей:
        ///1 сначала по длинне, если длина одинакова, то разбивать на символы и отталкиваться от численного значения
        ///2 сразу упорядовчивать по численному значению - тупо, но проще
